...
class SbpJobClosedNotification extends Mailable
{
    ...
    public function build()
    {
        ...
        $subject = "Surge Alert: ". $this->job->title .' - '  . $place . ' - ' . $this->organization;

        if(!empty($this->cluster) && !is_null($this->cluster)) {
          $subject = $subject . ' - ' . $this->cluster;
        }

        return $this->subject($subject)->markdown('mail.SbpJobClosedNotification');
    }
