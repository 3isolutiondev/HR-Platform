...
class Durations extends Component {
			...
			options: {
				...
				customToolbar: () => {
					if (can('Add Duration')) {
						return (
							<Btn
								link="/dashboard/durations/add"
								btnText="Add New Duration"
								btnStyle="contained"
								color="primary"
								size="small"
								icon={<Add />}
		...
		let dataInArray = jsonData.map((data, index) => {
			...
			const actions = (
				<div className={classes.actions}>
					{/* {can('Show Duration') && (
						<CircleBtn
							link={'/dashboard/durations/' + data.id}
							color={classes.blue}
							size="small"
							icon={<RemoveRedEye />}
						/>
					...
					{can('Edit Duration') && (
						<CircleBtn
							link={'/dashboard/durations/' + data.id + '/edit'}
	...
	render() {
		...
		return (
			<div>
				<Helmet>
					<title>{APP_NAME + ' - Dashboard > Duration List'}</title>
					<meta name="description" content={APP_NAME + ' Dashboard > Duration List'} />
				</Helmet>
				<MUIDataTable
					title={'Duration List'}
					data={roles}
				...
				<Alert
					...
					text={'Are you sure to delete duration : ' + name + ' ?'}
					closeText="Cancel"
					AgreeText="Yes"
				/>
			</div>
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(Durations));
