...
class HRJobCategories extends Component {
	constructor(props) {
		super(props);
		this.state = {
			hrJobCategories: [],
			...
			options: {
				...
				selectableRows: 'none',
				customToolbar: () => {
					if (can('Add HR Job Category')) {
						return (
							<Btn
								link="/dashboard/hr-job-categories/add"
								btnText="Add New Job Category"
								btnStyle="contained"
								color="primary"
								size="small"
								icon={<Add />}
		...
		this.approveJobCategory = this.approveJobCategory.bind(this);
	}
	...
	approveJobCategory(id, is_approved) {
		this.props
			.postAPI('/api/hr-job-categories/approve', {
				id,
				is_approved
			})
			.then((res) => {
	...
	dataToArray(jsonData) {
		...
		let dataInArray = jsonData.map((data, index) => {
			...
			const isApproved = (
				<div>
					<Checkbox
						...
						onChange={(e) => this.approveJobCategory(data.id, e.target.checked ? 1 : 0)}
			...
			dataTemp.push(isApproved);

			const actions = (
				<div className={classes.actions}>
					{/* {can('Show HR Job Category') && (
						<CircleBtn
							link={'/dashboard/hr-job-categories/' + data.id}
							color={classes.blue}
							size="small"
							icon={<RemoveRedEye />}
						/>
					)} */}
					{can('Edit HR Job Category') && (
						<CircleBtn
							link={'/dashboard/hr-job-categories/' + data.id + '/edit'}
							color={classes.purple}
							size="small"
							icon={<Edit />}
						/>
					)}
					{can('Delete HR Job Category') && (
						<CircleBtn
							color={classes.red}
							size="small"
							icon={<Delete />}
	...
	render() {
		...
		return (
			<div>
				<Helmet>
					<title>{APP_NAME + ' - Dashboard > Job Category List'}</title>
					<meta name="description" content={APP_NAME + ' Dashboard > Job Category List'} />
				</Helmet>
				<MUIDataTable
				...
				<Alert
					...
					text={'Are you sure to delete job category : ' + name + ' ?'}
					closeText="Cancel"
					AgreeText="Yes"
				/>
			</div>
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(HRJobCategories));
