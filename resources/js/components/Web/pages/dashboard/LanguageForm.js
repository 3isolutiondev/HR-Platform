...
class LanguageForm extends Component {
	constructor(props) {
		...
		this.state = {
			...
			redirectURL: '/dashboard/languages'
		};
	...
	componentWillMount() {
		if (typeof this.props.match.params.id !== 'undefined') {
			this.setState({
				isEdit: true,
				apiURL: '/api/languages/' + this.props.match.params.id,
				redirectURL: '/dashboard/languages/' + this.props.match.params.id + '/edit'
			});
			...
			this.props
				...
				.catch((err) => {
					this.props.addFlashMessage({
						type: 'error',
						text: 'There is an error while requesting language data'
					});
				});
		}
	}
	...
	onSubmit(e) {
		...
		if (this.state.isEdit) {
			languageData._method = 'PUT';
		}
		if (this.isValid()) {
			this.setState({ showLoading: true }, () => {
				this.props
					.postAPI(this.state.apiURL, languageData)
					.then((res) => {
		...
		return (
			...
			>
				<Helmet>
					<title>
						{isEdit ? (
							APP_NAME + ' - Dashboard > Edit Language : ' + name
						) : (
							APP_NAME + ' - Dashboard > Add Language'
						)}
					</title>
					<meta
						name="description"
...
export default withStyles(styles)(connect(null, mapDispatchToProps)(LanguageForm));
