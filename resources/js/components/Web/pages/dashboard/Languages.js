...
class Languages extends Component {
		...
		let dataInArray = jsonData.map((data, index) => {
			...
			const actions = (
				<div className={classes.actions}>
					{/* {can('Show Language') && (
						<CircleBtn
							link={'/dashboard/languages/' + data.id}
							color={classes.blue}
							size="small"
							icon={<RemoveRedEye />}
						/>
					...
					{can('Edit Language') && (
						<CircleBtn
							link={'/dashboard/languages/' + data.id + '/edit'}
	...
	render() {
		...
		return (
			<div>
				<Helmet>
					<title>{APP_NAME + ' - Dashboard > Language List'}</title>
					<meta name="description" content={APP_NAME + ' Dashboard > Language List'} />
				</Helmet>
				<MUIDataTable
					title={'Language List'}
					data={roles}
