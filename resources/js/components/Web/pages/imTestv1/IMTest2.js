...
class IMTest2 extends Component {
	componentDidMount() {
		this.props.isValid(3);
	}
	componentDidUpdate(prevProps) {
		// const currentDashboardIMTest = JSON.stringify(this.props.imtest.step3);
		// const prevDashboardIMTest = JSON.stringify(prevProps.imtest.step3);

		// if (currentDashboardIMTest !== prevDashboardIMTest) {
		// 	this.props.isValid(3);
		// }
		if (this.props.imtest.step3 !== prevProps.imtest.step3) {
			this.props.isValid(3);
		}
	}
	render() {
		let { text1, text2, title, text3, text4, file_dataset1, file_dataset2, questions } = this.props.imtest.step3;
				...
				{isAdd || isEdit ? (
					<Grid container spacing={16}>
						<Grid item xs={12}>
							<TextField
								...
								value={title}
								autoComplete="title"
								onChange={(e) => onChangeStep('step3', e.target.name, e.target.value)}
						...
						<Grid item>
							<DropzoneFileField
								name="file_dataset1"
								label="Dataset 1"
								onUpload={(name, data) => onUpload(name, data, 'step3')}
								onDelete={(name, apiUrl, data, deleteId) =>
									onDelete(name, apiUrl, data, deleteId, 'step3', file_dataset1.model_id)}
										...
										<Grid item xs={12}>
											<TextField
												...
												onChange={(e) => {
													...
													onChangeStep('step3', 'questions', questions);
												}}
						...
						<Grid item>
							<DropzoneFileField
								...
								onUpload={(name, data) => onUpload(name, data, 'step3')}
								onDelete={(name, apiUrl, data, deleteId) =>
									onDelete(name, apiUrl, data, deleteId, 'step3', file_dataset2.model_id)}
					...
					<Grid container spacing={16}>
						...
						<Grid container xs={12}>
							...
							<Grid item xs={3}>
								<FormControl margin={margin} fullWidth={fullWidth} error={!isEmpty(errors.upload1)}>
									...
									{!isEmpty(errors.upload1) && <FormHelperText>{errors.upload1}</FormHelperText>}
								</FormControl>
							</Grid>
						</Grid>
						<Grid item xs={12}>
						...
						<Grid container xs={12}>
							...
							<Grid item xs={3} className={classes.addMarginBottom}>
								<FormControl margin={margin} fullWidth={fullWidth} error={!isEmpty(errors.upload3)}>
									<FormLabel>Upload</FormLabel>
									<Button
										variant="contained"
										color="primary"
									...
									{!isEmpty(errors.upload3) && <FormHelperText>{errors.upload3}</FormHelperText>}
								</FormControl>
							</Grid>
						</Grid>
					</Grid>
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(IMTest2));
