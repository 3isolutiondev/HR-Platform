...
class IMTest4 extends Component {
	componentDidMount() {
		this.props.isValid(5);
	}
	componentDidUpdate(prevProps) {
		const currentDashboardIMTest = JSON.stringify(this.props.imtest.step5);
		const prevDashboardIMTest = JSON.stringify(prevProps.imtest.step5);
	...
	render() {
		let { text1, title, answer } = this.props.imtest.step5;
					...
					<Grid container spacing={16}>
						<Grid item xs={12}>
							<TextField
								...
								value={title}
								autoComplete="title"
								onChange={(e) => onChangeStep('step5', e.target.name, e.target.value)}
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(IMTest4));
