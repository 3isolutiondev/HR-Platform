...
import PastReferenceCheck from './PastReferenceCheck';
...
class ReferenceCheck extends Component {
	constructor(props) {
		...
		this.state = {
			reference_check: '',
			alreadySent: props.user.job_user[0].reference_check_sent === 1 ? true : false,
			isLoading: false,
			p11ReferenceId: null,
	...
	/**
   * toogleReferenceHistory toogles the value of showReferenceHistroy
   */
	...
	/**
   * referenceCheckInvitation sends a reference check invitation to the user
   */
	referenceCheckInvitation() {
		const { p11ReferenceId } = this.state;
		const { user } = this.props;
		const { profile, selected_step, job_user } = user;
		const { job } = job_user[0];
		// let { has_reference_check, id, order } = selected_step;
		this.setState({ isLoading: true }, () => {
			const data = {
				jobId: job.id,
				// has_reference_check: has_reference_check,
				current_step: job_user[0].job_status_id,
				profile_id: profile.id,
				job_user_id: job_user[0].id,
			};
			if (p11ReferenceId) data.p11_reference_id = this.state.p11ReferenceId;
			axios
				.post('/api/jobs/reference-check-invitation', data)
				.then((res) => {
	...
	render() {
		const { classes, user, isReference } = this.props;
		const { profile } = user;
		const { alreadySent, isLoading } = this.state;
		const canSendReferenceCheck = can('Send Reference Check');
		const gridWith = canSendReferenceCheck ? 6 : 12;
		const oldJob = profile.p11_references.filter((reference) => reference.reference_file).length === 0 && !isReference;

		return oldJob ? null : (
		 <>
			{this.props.divider}
			<Grid container spacing={16} alignItems="center" className={classes.inviteContainer}>
			{canSendReferenceCheck && <Grid item xs={12} md={6} lg={6} xl={6}>
				<Button
				size="small"
				color="primary"
				fullWidth
			...
			<Grid item xs={12} md={gridWith} lg={gridWith} xl={gridWith}>
				{(canSendReferenceCheck || alreadySent) && <Button
				size="small"
				color="primary"
				fullWidth
				variant="contained"
			...
			{(profile.p11_references.length > 0 && canSendReferenceCheck && !oldJob) && <>
			...
			<Grid container spacing={16} alignItems="center" className={classes.inviteContainer}>
				<Grid item xs={12} md={12} lg={12} xl={12}>
					{
						profile.p11_references.map(reference => {
