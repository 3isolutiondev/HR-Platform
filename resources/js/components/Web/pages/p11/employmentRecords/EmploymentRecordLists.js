...
class EmploymentRecordLists extends Component {
	constructor(props) {
		super(props);
	...
	dataToArray(jsonData) {
		...
		let dataInArray = jsonData.map((data, index) => {
			let dataTemp = [ data.id, data.from, data.to, data.country.name, data.employer_name, data.job_title ];
		...
		return (
			<div>
				...
				<Alert
					...
					text={'Are you sure to delete your employment history with employer name : ' + name + ' ?'}
					closeText="Cancel"
					AgreeText="Yes"
				/>
			</div>
...
EmploymentRecordLists.propTypes = {
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(EmploymentRecordLists));
