...
class ProfessionalSocietyLists extends Component {
	constructor(props) {
		super(props);
	...
	dataToArray(jsonData) {
		...
		let dataInArray = jsonData.map((data, index) => {
			let dataTemp = [ data.id, data.attended_from, data.attended_to, data.country.name, data.name ];
		...
		return (
			<div>
				<ProfessionalSocietyForm
					isOpen={openDialog}
					recordId={dataId}
					title="Professional Societies and Activities in Civic, Public or International Affairs"
				...
				/>
				<MUIDataTable
					title="List Professional Societies and Activities in Civic, Public or International Affairs"
					data={records}
					columns={columns}
					options={options}
					download={false}
...
ProfessionalSocietyLists.propTypes = {
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(ProfessionalSocietyLists));
