...
class ReferenceLists extends Component {
				...
				customToolbar: () => {
					return (
						<Button
							...
							disabled={this.props.references_counts >= 3 ? true : false}
						>
							<Add />
							Add Reference
						</Button>
	...
	dataToArray(jsonData) {
		...
		let dataInArray = jsonData.map((data, index) => {
			let dataTemp = [ data.id, data.full_name, data.organization, data.phone, data.email ];
	...
	dialogOpen() {
		if(this.props.checkNoticeRead && !this.props.noticeRead) {
	...
	render() {
		...
		return (
			<div>
				{this.state.showReferenceNoticeModal && <ReferenceNoticeModal isOpen onClose={() => {
					this.setState({ showReferenceNoticeModal: false, openDialog: true });
				}} />}
				<ReferenceForm
					isOpen={openDialog}
					recordId={dataId}
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(ReferenceLists));
