...
class ReferenceQuestion extends Component {
	...
	dataToArray(jsonData) {
		...
		let usersInArray = jsonData.map((user, index) => {
			let userTemp = [ user.id, user.belongscategory.name, user.question ];

			const actions = (
				<div className={classes.actions}>
					{can('Show Question Category') ? (
						<CircleBtn
							link={'/reference-question/' + user.id}
							color={classes.blue}
							size="small"
							icon={<RemoveRedEye />}
						/>
					...
					{can('Edit User') ? (
						<CircleBtn
							link={'/reference-question/' + user.id + '/edit'}
...
export default withStyles(styles)(connect('', mapDispatchToProps)(ReferenceQuestion));
