...
import { getRosters } from '../../../actions/dashboard/roster/rosterDashboardActions';
...
export const filterCheckbox = (name, value, variableIsOpen, isChecked, parameterVariable, usingDialog) => (
	...
	if (dataIndex < 0 && isChecked && usingDialog) {
		let paramValue = JSON.parse(JSON.stringify(getState().filter[parameterVariable]));
		paramValue.id = value;
...
export const immaperOnChange = (value, isChecked) => (dispatch, getState) => {
	let immaper_status = JSON.parse(JSON.stringify(getState().filter.immaper_status));
...
export const availableOnChange = (value, isChecked) => (dispatch, getState) => {
	let is_available = JSON.parse(JSON.stringify(getState().filter.is_available));
...
export const setParameters = (variable_name, parameters, variableIsOpen) => (dispatch, getState) => {
	let chosen_data = JSON.parse(JSON.stringify(getState().filter[variable_name]));
