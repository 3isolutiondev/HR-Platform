import { SET_TOR_FILTER_DATA, RESET_TOR_FILTER } from '../../types/tor/torFilterTypes';
...
export const immaperOnChange = (value, isChecked) => (dispatch, getState) => {
	let immaper_status = JSON.parse(JSON.stringify(getState().torFilter.immaper_status));
...
export const genderOnChange = (value, isChecked) => (dispatch, getState) => {
	let select_gender = JSON.parse(JSON.stringify(getState().torFilter.select_gender));
...
export const availableOnChange = (value, isChecked) => (dispatch, getState) => {
	let is_available = JSON.parse(JSON.stringify(getState().torFilter.is_available));
...
export const resetFilter = () => (dispatch, getState) => {
	dispatch({ type: RESET_TOR_FILTER });
	// dispatch(getFilter());
	let {
		degree_levels,
		languages,
...
export const filterCheckbox = (name, value, variableIsOpen, isChecked, parameterVariable, usingDialog) => (
	...
	let filterData = JSON.parse(JSON.stringify(getState().torFilter[name]));
	let filterDataId = pluck(filterData, 'id');
	let dataIndex = filterDataId.indexOf(value);

	if (dataIndex < 0 && isChecked && usingDialog) {
		let paramValue = JSON.parse(JSON.stringify(getState().torFilter[parameterVariable]));
		paramValue.id = value;
		filterData.push(paramValue);
		dispatch(onChange(variableIsOpen, getState().torFilter[variableIsOpen] ? false : true));
...
// };
export const setParameters = (variable_name, parameters, variableIsOpen) => (dispatch, getState) => {
	let chosen_data = JSON.parse(JSON.stringify(getState().torFilter[variable_name]));
	let arrIds = pluck(chosen_data, 'id');
...
export const closeDialog = (variableIsOpen) => (dispatch, getState) => {
	let isOpen = getState().torFilter[variableIsOpen];
