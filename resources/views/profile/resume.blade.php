    ...
    <body>
        ...
        <div class="identity">
            <div class="identity-left">
                <h1 class="font-light text-name text-immap-secondary">{{ $profile->user->first_name }} @if($profile->user->middle_name) {{ $profile->user->middle_name }} @endif </h1>
                <h1 class="font-bold text-name text-immap-secondary text-uppercase">{{ $profile->user->family_name }}</h1>
                @php
            ...
            <div class="identity-right">
                @if($photo !== '')
                    <img src="{{ $photo }}" class="avatar" width="175" style="float: right;"/>
                @endif
            </div>
        </div>
        <div class="detail-container">
            <div class="left-column column-padding">
                ...
                @endif
                {{-- Contact --}}
                @include('profile.section-title', ['fullwidth' => false, 'title' => 'Contact'])
                @if(!empty($profile->phones))
                    @if(count($profile->phones))
                        @php
                            $primaryPhone = $profile->phones()->where('is_primary', 1)->first();
                            if (empty($primaryPhone)) {
                                $primaryPhone = $profile->phones->first();
                            }
                        @endphp
                ...
                @if(!empty($profile->p11_references))
                    @if(count($profile->p11_references))
                        @include('profile.section-title', ['fullwidth' => false, 'title' => 'Reference'])
                        <div class="margin-bottom-lg margin-top-lg">
                            @foreach($profile->p11_references as $reference)
                                <div class="margin-bottom-md">
                                    <h5 class="text-darker-grey font-bold text-uppercase">{{ $reference->full_name }}</h5>
                                    <div class="text-darker-grey margin-bottom-sm">{{ $reference->job_position }}, {{ $reference->organization }}</div>
                                    @if(!empty($reference->phone) && !is_null($reference->phone))
                                        <table>
                                            <tbody>
                                                <tr>
                                                    <td class="text-darker-grey font-bold" style="width: 48px;">Phone</td>
                                                    <td class="text-darker-grey font-bold">:</td>
                                                    <td><div class="margin-left-xs">{{ $reference->phone }}</div></td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    @endif
                                    @if(!empty($reference->email) && !is_null($reference->email))
                                        <table>
                                            <tbody>
                                                <tr>
                                                    <td class="text-darker-grey font-bold" style="width: 48px;">Email</td>
                                                    <td class="text-darker-grey font-bold">:</td>
                                                    <td><div class="margin-left-xs">{{ $reference->email }}</div></td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    @endif
